...
- Configure SMTP credentials and the notification recipient from `/admin` -> Email (includes a "Send test email" action), or set `SMTP_HOST`, `SMTP_USER`, `SMTP_PASSWORD`, `INQUIRY_EMAIL_TO`, and optionally `SMTP_PORT`, `SMTP_SECURE`, `INQUIRY_EMAIL_FROM` as env-var fallbacks. Admin-panel values take priority when both are set.
