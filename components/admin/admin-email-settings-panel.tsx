...
export function AdminEmailSettingsPanel() {
  ...
  const formattedVerifiedAt = formatUpdatedAt(verifiedAt);
      ...
      {loading ? (
        ...
        <>
          <div
            role="status"
            aria-live="polite"
            className={`flex items-center gap-2.5 rounded-2xl px-4 py-3.5 text-sm ${
              isVerified
          ...
          >
            ...
            <span>
              {isVerified
                ? `Working — confirmed by a successful test${formattedVerifiedAt ? ` on ${formattedVerifiedAt}` : ""}.`
                : `Not working yet. Inquiries are still saved, but no email notification will be sent until you save these settings and a "Send test email" succeeds.`}
            </span>
          </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="space-y-2 text-xs uppercase tracking-meta text-muted">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField("inquiryEmailTo", e.target.value)
                  }
